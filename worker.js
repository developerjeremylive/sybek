  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    input, textarea, select { 
      width: 100%; background: #12121a; border: 1px solid #2d2d3a; border-radius: 8px; 
      padding: 12px; color: #e2e8f0; font-size: 14px; margin-bottom: 12px; 
    }
...
<body>
  <div class="container">
    ...
    <div class="sidebar">
      <div class="logo">
        ...
        <div>
          <div class="logo-text">Sybek</div>
          <div class="logo-sub">Deploy + Monitor</div>
        </div>
      </div>
      <nav>
      ...
      <div id="section-deploy" class="section active">
        ...
        <div class="card">
          ...
          <div class="form-group">
            ...
            <input type="text" id="workerName" placeholder="my-worker">
          </div>
          <div class="form-group">
            <label>JavaScript Code</label>
            <textarea id="workerCode" rows="6" placeholder="addEventListener('fetch', event => {...})" style="font-family:monospace"></textarea>
          </div>
          <button class="btn btn-primary" onclick="deployWorker()"><i class="fas fa-upload"></i> Deploy</button>
          <button class="btn btn-secondary" onclick="loadExample()"><i class="fas fa-code"></i> Example</button>
        </div>
      ...
      <div id="section-workers" class="section">
        ...
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
            ...
            <button class="btn btn-primary" onclick="loadWorkers()"><i class="fas fa-sync"></i> Refresh</button>
          </div>
          <div id="workersList">
            <div style="text-align:center;padding:40px;color:#94a3b8">
              <i class="fas fa-cloud-upload-alt" style="font-size:40px;margin-bottom:10px"></i>
              <p>No workers deployed</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="section-openclaw" class="section">
        ...
        <div class="card">
          <div style="display:flex;align-items:center;gap:16px">
            <div style="width:60px;height:60px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:50%;display:flex;align-items:center;justify-content:center">
              <i class="fas fa-robot" style="color:white;font-size:28px"></i>
            </div>
            <div>
              <h2><span class="status-dot" id="ocStatusDot"></span>Gateway</h2>
              <p style="color:#94a3b8;font-size:14px" id="ocUrl">Not connected</p>
            </div>
            <button class="btn btn-primary" style="margin-left:auto" onclick="connectOpenClaw()"><i class="fas fa-sync"></i> Connect</button>
          </div>
        </div>
      ...
      <div id="section-logs" class="section">
        ...
        <div class="card">
          <div style="display:flex;gap:12px;margin-bottom:16px">
            <select id="logFilter" style="width:auto">
      ...
      <div id="section-settings" class="section">
        ...
        <div class="card">
          ...
          <div class="form-group">
            ...
            <input type="password" id="cfToken" placeholder="Your Cloudflare API Token">
          </div>
          <div class="form-group">
        ...
        <div class="card">
          ...
          <div class="form-group">
            ...
            <input type="text" id="openclawUrl" placeholder="https://claw.kilosessions.ai">
          </div>
          <div class="form-group">
            <label>Gateway Token</label>
            <input type="password" id="openclawToken" placeholder="Your Gateway Token">
          </div>
          <button class="btn btn-primary" onclick="testConnection()"><i class="fas fa-plug"></i> Test</button>
        </div>
      </div>
  ...
  <script>
    ...
    const kiloToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbnYiOiJwcm9kdWN0aW9uIiwia2lsbFVzZXJJZCI6ImVjNGVlZjQ5LWUyM2MtNDhhNy05ODhjLTI3ZWIwOWNkYTFhMSIsImFwaVRva2VuUGVwcGVyIjpudWxsLCJ2ZXJzaW9uIjozLCJpYXQiOjE3NzI0MzExNzAsImV4cCI6MTkzMDExMTE3MH0.wfGWzGMdNYFR_SpVHbUWoDSSv5pCKck1PeCxNg6DlaM';
    ...
    function loadWorkers() {
      ...
      list.innerHTML = workers.map(w => '<div class="channel-item"><div style="flex:1"><div>' + w.name + '</div><div style="font-size:12px;color:#94a3b8">https://' + w.name + '.workers.dev</div></div><span class="status-dot status-online"></span></div>').join('');
    }

    // OpenClaw
    function connectOpenClaw() {
      const ocUrl = document.getElementById('openclawUrl').value || kiloUrl;
      const token = document.getElementById('openclawToken').value || kiloToken;
      ...
      fetch('/api/openclaw/status?url=' + encodeURIComponent(ocUrl) + '&token=' + encodeURIComponent(token))
        ...
        .then(data => {
          if (data.connected) {
            ...
            const tg = data.channels?.find(c => c.provider === 'telegram' || c.id === 'telegram');
            if (tg) {
      ...
      fetch('/api/telegram/messages?url=' + encodeURIComponent(ocUrl) + '&token=' + encodeURIComponent(token))
        ...
        .then(data => {
          ...
          if (msgs.length === 0) {
            ...
            document.getElementById('telegramMessages').innerHTML = msgs.map(m => '<div class="msg-item"><div class="msg-header"><span class="msg-sender">' + m.sender + '</span><span class="msg-time">' + m.time + '</span></div><div class="msg-content">' + m.content + '</div></div>').join('');
          }
        })
        .catch(e => {
    ...
    function refreshLogs() {
      ...
      const icons = {deploy: 'fa-cloud-upload-alt', openclaw: 'fa-robot', telegram: 'fa-telegram', error: 'fa-exclamation-circle'};
      const colors = {deploy: '#6366f1', openclaw: '#6366f1', telegram: '#229ED9', error: '#ef4444'};
      
      list.innerHTML = filtered.map(l => '<div class="log-item"><i class="fas ' + icons[l.type] + ' log-icon" style="color:' + colors[l.type] + '"></i><div><div>' + l.message + '</div><div class="log-time">' + l.time + '</div></div></div>').join('');
    }
...
async function handleRequest(request, env) {
  ...
  if (url.pathname === '/api/deploy' && request.method === 'POST') {
    try {
      ...
      return new Response(JSON.stringify({ success: data.success || false, error: data.errors?.[0]?.message }), {
        status: 200, headers: { 'Content-Type': 'application/json', ...cors }
      });
    } catch (e) {
      return new Response(JSON.stringify({ success: false, error: e.message }), {
