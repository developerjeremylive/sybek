...
'.sidebar { width: 240px; background: #12121a; border-right: 1px solid #2d2d3a; padding: 20px; }' +
...
'.nav-item { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 10px; color: #94a3b8; cursor: pointer; margin-bottom: 4px; }' +
...
'input, textarea, select { width: 100%; background: #12121a; border: 1px solid #2d2d3a; border-radius: 8px; padding: 12px; color: #e2e8f0; margin-bottom: 12px; }' +
'.btn { padding: 12px 20px; border-radius: 8px; border: none; cursor: pointer; font-size: 14px; display: inline-flex; align-items: center; gap: 8px; }' +
...
'.channel-icon { width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; }' +
...
'.log-item { padding: 10px 0; border-bottom: 1px solid #2d2d3a; font-size: 13px; }' +
'@media (max-width: 768px) { .container { flex-direction: column; } .sidebar { width: 100%; } .stats { grid-template-columns: 1fr; } }' +
'</style>' +
'</head>' +
'<body>' +
'<div class="container">' +
'<div class="sidebar">' +
'<div class="logo"><div class="logo-icon">S</div><div><div class="logo-text">Sybek</div><div class="logo-sub">Deploy + Monitor</div></div></div>' +
'<nav>' +
...
'</nav>' +
'<div style="margin-top:auto;padding-top:20px;border-top:1px solid #2d2d3a;font-size:12px;color:#94a3b8"><span class="status-dot status-online"></span>Cloudflare</div>' +
'</div>' +
'<div class="main">' +

// DEPLOY SECTION
'<div id="section-deploy" class="section active">' +
'<h1><i class="fas fa-rocket"></i> Deploy Worker</h1>' +
'<div class="stats">' +
...
'<div class="form-group"><label>JavaScript Code</label><textarea id="workerCode" rows="6" placeholder="addEventListener..."></textarea></div>' +
...
'</div>' +
'<div class="card" id="deployResult" style="display:none"><h2>Result</h2><pre id="deployOutput" style="background:#12121a;padding:16px;border-radius:8px;overflow-x:auto"></pre></div>' +
'</div>' +

// WORKERS SECTION
'<div id="section-workers" class="section">' +
'<h1><i class="fas fa-server"></i> Workers</h1>' +
'<div class="card"><h2>Deployed Workers</h2><div id="workersList">No workers</div></div>' +
'</div>' +

// OPENCLAW SECTION
'<div id="section-openclaw" class="section">' +
'<h1><i class="fas fa-robot"></i> OpenClaw</h1>' +
'<div class="card">' +
...
'<div><h2><span class="status-dot" id="ocStatusDot"></span>Gateway</h2><p style="color:#94a3b8" id="ocUrl">Not connected</p></div>' +
'<button class="btn btn-primary" id="btnConnect" style="margin-left:auto">Connect</button>' +
'</div>' +
'</div>' +
'<div class="stats">' +
...
'</div>' +
'<div class="card"><h2>Channels</h2><div id="channelsList"><div class="channel-item"><div class="channel-icon" style="background:#229ED9"><i class="fab fa-telegram" style="color:white"></i></div><div style="flex:1"><div>Telegram</div><div style="font-size:12px;color:#94a3b8" id="tgStatus">Checking...</div></div><span id="tgBadge" class="status-dot status-loading"></span></div></div></div>' +
'</div>' +

// TELEGRAM SECTION
'<div id="section-telegram" class="section">' +
'<h1><i class="fab fa-telegram"></i> Telegram</h1>' +
'<div class="stats">' +
...
'</div>' +
'<div class="card"><h2>Recent Messages</h2><div id="telegramMessages">Loading...</div></div>' +
'</div>' +

// LOGS SECTION
'<div id="section-logs" class="section">' +
'<h1><i class="fas fa-list"></i> Logs</h1>' +
'<div class="card"><div id="logsList">No logs</div></div>' +
'</div>' +

// SETTINGS SECTION
'<div id="section-settings" class="section">' +
'<h1><i class="fas fa-cog"></i> Settings</h1>' +
'<div class="card"><h2>Cloudflare</h2><div class="form-group"><label>API Token</label><input type="password" id="cfToken"></div><div class="form-group"><label>Account ID</label><input type="text" id="cfAccountId"></div><button class="btn btn-primary" id="btnSaveSettings">Save</button></div>' +
'<div class="card"><h2>OpenClaw</h2><div class="form-group"><label>Gateway URL</label><input type="text" id="openclawUrl" value="https://claw.kilosessions.ai"></div><div class="form-group"><label>Gateway Token</label><input type="password" id="openclawToken"></div><button class="btn btn-primary" id="btnTestConnection">Test Connection</button></div>' +
'</div>' +
...
// Nav
'document.querySelectorAll(".nav-item").forEach(function(item) { item.onclick = function() { showSection(item.dataset.section); }; });' +
...
'  .then(function(data) {' +
'    if (data.success) { output.textContent += "\\nDeployed!"; workers.unshift({name: name}); addLog("deploy", "Worker deployed"); }' +
'    else { output.textContent += "\\nError: " + (data.error || "Unknown"); addLog("error", "Deploy failed"); }' +
...
'  var url = (document.getElementById("openclawUrl").value || kiloUrl).replace(/\\/*$/, "");' +
...
'  document.getElementById("ocUrl").textContent = url;' +
...
'    if (msgs.length === 0) { document.getElementById("telegramMessages").innerHTML = "No messages"; }' +
'    else { document.getElementById("telegramMessages").innerHTML = msgs.map(function(m) { return "<div class=\\"msg-item\\"><div class=\\"msg-header\\"><span class=\\"msg-sender\\">" + m.sender + "</span><span class=\\"msg-time\\">" + m.time + "</span></div><div class=\\"msg-content\\">" + m.content + "</div></div>"; }).join(""); }' +
'  }).catch(function(e) { document.getElementById("telegramMessages").innerHTML = "Error: " + e.message; });' +
'}' +

'function addLog(type, msg) {' +
'  logs.unshift({type: type, message: msg, time: new Date().toLocaleTimeString()});' +
'  var icons = {deploy: "fa-cloud-upload-alt", openclaw: "fa-robot", error: "fa-exclamation-circle"};' +
'  var colors = {deploy: "#6366f1", openclaw: "#6366f1", error: "#ef4444"};' +
'  document.getElementById("logsList").innerHTML = logs.map(function(l) { return "<div class=\\"log-item\\"><i class=\\"fas " + icons[l.type] + "\\" style=\\"color:" + colors[l.type] + "\\"></i> " + l.message + " <span style=\\"color:#94a3b8\\">" + l.time + "</span></div>"; }).join("");' +
'}' +

'function saveSettings() {' +
'  localStorage.setItem("sybek_settings", JSON.stringify({' +
...
'    document.getElementById("openclawUrl").value = s.openclawUrl || "https://claw.kilosessions.ai";' +
...
'  .then(function(r) { return r.json(); })' +
'  .then(function(d) {' +
'    if (d.success) { alert("Connected!"); loadSettings(); connectOpenClaw(); }' +
...
async function handleRequest(request) {
  ...
  if (url.pathname === '/api/openclaw/status' || url.pathname === '/api/openclaw/test') {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return new Response(JSON.stringify({
          ...
          sessions: 1,
          channels: [{id: 'telegram', provider: 'telegram', name: '@developerjeremylive'}]
      ...
      if (response.ok) {
        return new Response(JSON.stringify({
          ...
          messages: [
            ...
            {sender: 'User 2', content: 'Testing the bot', time: '2m'}
          ]
        }), { status: 200, headers: { 'Content-Type': 'application/json', ...cors } });
      } else {
