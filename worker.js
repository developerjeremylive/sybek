...
'<div class="card"><input id="curl" value="https://claw.kilosessions.ai"><input id="ctoken" type="password" placeholder="Token">',
'<button class="btn" id="btest">Test</button></div></div>',
...
'function cfg(){var s=JSON.parse(localStorage.getItem("c")||"{}");document.getElementById("curl").value=s.u||"https://claw.kilosessions.ai";document.getElementById("ctoken").value=s.t||"";return s;}',
'document.getElementById("btest").onclick=function(){var u=document.getElementById("curl").value.replace(/\\/*$/,"");localStorage.setItem("c",JSON.stringify({u:u,t:document.getElementById("ctoken").value}));log("Testing: "+u);document.getElementById("ocdot").className="dot yellow";fetch("/api/test?u="+encodeURIComponent(u)).then(function(r){return r.json()}).then(function(d){if(d.ok){log("Connected!","info");loadOC();}else{log("Failed: "+d.error,"error");}}).catch(function(e){log("Error: "+e,"error");});};',
