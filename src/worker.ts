...
export default {
  async fetch(request: Request, env: any, ctx: any): Promise<Response> {
    ...
    // MCPorter API endpoints
    if (url.pathname.startsWith('/api/mcporter/')) {
      return handleMcporterRequest(request, url);
    }
...
// MCPorter request handler
async function handleMcporterRequest(request: Request, url: URL): Promise<Response> {
  const pathParts = url.pathname.replace('/api/mcporter/', '').split('/');
  const serverName = pathParts[0];
  const endpoint = pathParts[1] || '';

  // Install server endpoint
  if (url.pathname === '/api/mcporter/install' && request.method === 'POST') {
    try {
      const body = await request.json();
      const server = body.server || serverName;
      
      // Run mcporter install commands in background
      // This would typically spawn the MCP server
      const installCommands = [
        `mcporter add ${server}`,
        `mcporter start ${server}`
      ];
      
      // For now, return success - actual installation happens via terminal
      return new Response(JSON.stringify({ 
        success: true, 
        message: `Server ${server} installation initiated`,
  ...
  // List tools endpoint
  if (endpoint === 'tools' || url.pathname === `/api/mcporter/${serverName}`) {
    try {
      // Call mcporter to get tools
      // Using a simulated response for now - in production this would call mcporter
  ...
  // Call tool endpoint
  if (endpoint === 'call' && request.method === 'POST') {
    try {
      const body = await request.json();
      const { tool, args = {} } = body;
      
      const result = await callMcpTool(serverName, tool, args);
  ...
  if (serverName === 'puppeteer') {
    return [
      ...
      },
      {
        ...
        description: 'Ejecuta JavaScript en la página',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description: 'Obtiene el HTML actual de la página',
        inputSchema: {
          type: 'object',
          properties: {}
        }
      ...
      {
        name: 'browser_urls',
        description: 'Lista todas las URLs de las pestañas abiertas',
        inputSchema: {
          type: 'object',
          properties: {}
        }
      },
      {
        ...
        inputSchema: {
          type: 'object',
          properties: {
            url: { type: 'string', description: 'URL opcional' }
          }
        }
      },
      {
        name: 'browser_close_tab',
        description: 'Cierra una pestaña',
        inputSchema: {
          type: 'object',
          properties: {
            tabId: { type: 'string', description: 'ID de la pestaña a cerrar' }
          }
        }
      }
    ];
