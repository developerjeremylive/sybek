...
import { Send, Wrench, X, Bot, Check, Plug, ChevronDown, Zap, ArrowRight } from 'lucide-react';
...
export function MCPSelector() {
  ...
  const toolsByServer = activeTools.reduce((acc, tool) => {
    if (!acc[tool.serverName]) acc[tool.serverName] = [];
    acc[tool.serverName].push(tool);
    return acc;
  }, {} as Record<string, MCPTool[]>);

  // Show warning if servers are enabled but no tools (likely invalid URLs)
  const hasServersWithNoTools = enabledServers.length > 0 && activeTools.length === 0;

  if (enabledServers.length === 0) {
    return (
      <button
        onClick={() => window.location.href = '/mcp'}
        className="flex items-center gap-1.5 px-2.5 py-1.5 bg-zinc-800/50 hover:bg-zinc-700/50 rounded-lg text-xs transition-all border border-zinc-700/50 text-zinc-400 hover:text-zinc-300 cursor-pointer"
  ...
  // If servers enabled but no tools, show warning state
  if (hasServersWithNoTools) {
    return (
      <button
        onClick={() => window.location.href = '/mcp'}
        className="flex items-center gap-1.5 px-2.5 py-1.5 bg-yellow-500/10 hover:bg-yellow-500/20 rounded-lg text-xs transition-all border border-yellow-500/30 text-yellow-400 cursor-pointer"
        title="Servidores activos pero sin tools - verifica las URLs"
      >
        <Zap className="w-3.5 h-3.5" />
        <span className="hidden sm:inline">MCP</span>
        <span className="hidden sm:inline text-yellow-300/70">sin tools</span>
        <ChevronDown className="w-3 h-3 opacity-50" />
      </button>
    );
  }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        onClick={() => setShowDropdown(!showDropdown)}
        className="flex items-center gap-1.5 px-2.5 py-1.5 bg-gradient-to-r from-green-500/15 to-emerald-500/10 hover:from-green-500/20 hover:to-emerald-500/15 rounded-lg text-xs transition-all border border-green-500/30 hover:border-green-400/50 text-green-400 cursor-pointer shadow-sm"
        title={`${enabledServers.length} servidor(es), ${activeTools.length} tools disponibles`}
      >
        <div className="flex items-center gap-1">
          <Zap className="w-3.5 h-3.5" />
          <span className="hidden sm:inline max-w-[80px] truncate font-medium">
            {enabledServers[0]?.name}
          </span>
          {enabledServers.length > 1 && (
            <span className="text-[10px] bg-green-500/20 px-1.5 rounded-full">+{enabledServers.length - 1}</span>
          )}
        </div>
        <div className="hidden sm:flex items-center gap-1 text-green-300/70">
          <span>•</span>
          <span>{activeTools.length > 0 ? `${activeTools.length} tools` : 'sin tools'}</span>
        </div>
        <ChevronDown className={`w-3 h-3 transition-transform ${showDropdown ? 'rotate-180' : ''}`} />
      </button>
      
      {showDropdown && (
        <div className="absolute bottom-full mb-2 left-0 bg-zinc-900/95 backdrop-blur-sm border border-zinc-700/80 rounded-xl shadow-2xl z-50 w-64 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200">
          {/* Header */}
          <div className="px-3 py-2.5 border-b border-zinc-800 bg-zinc-800/30 flex items-center justify-between">
            <span className="text-xs font-medium text-zinc-300">MCP Servers</span>
          ...
          <div className="max-h-64 overflow-y-auto">
            {/* Show enabled servers even if no tools */}
            {enabledServers.length > 0 && (
              <div className="border-b border-zinc-800/50">
                <div className="flex items-center gap-2 px-3 py-2 bg-zinc-800/20">
                  <span className="text-sm">🔌</span>
                  <span className="text-xs font-medium text-zinc-300 flex-1">Servidores activos</span>
                </div>
                <div className="px-3 py-1.5 space-y-1">
                  {enabledServers.map((server) => {
                    const serverTools = activeTools.filter(t => t.serverId === server.id);
                    return (
                      <div key={server.id} className="flex items-center justify-between text-xs">
                        <span className="text-zinc-400">{server.name}</span>
                        <span className={`${serverTools.length > 0 ? 'text-green-400' : 'text-yellow-500'}`}>
                          {serverTools.length > 0 ? `${serverTools.length} tools` : 'sin tools'}
                        </span>
                      </div>
                    );
                  })}
            ...
            {Object.entries(toolsByServer).length > 0 && Object.entries(toolsByServer).map(([serverName, tools]) => (
              <div key={serverName} className="border-b border-zinc-800/50 last:border-0">
                <div className="flex items-center gap-2 px-3 py-2 bg-zinc-800/20">
                  <span className="text-sm">🔌</span>
                  <span className="text-xs font-medium text-zinc-300 flex-1">{serverName}</span>
                  <span className="text-[10px] text-zinc-500">{tools.length} tools</span>
                </div>
                <div className="px-3 py-1.5 space-y-0.5">
                  {tools.slice(0, 5).map((tool) => (
                    <div key={tool.name} className="text-[11px] text-zinc-400 truncate pl-4">
                      {tool.name}
                    </div>
                  ))}
                  {tools.length > 5 && (
          ...
          <a
            href="/mcp"
            className="flex items-center justify-center gap-2 px-3 py-2.5 text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50 border-t border-zinc-800 transition-colors"
          >
